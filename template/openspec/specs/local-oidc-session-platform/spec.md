# local-oidc-session-platform Specification

## Purpose

Defines local browser OIDC login, logout, Redis-backed session tickets, and the
Aspire local identity-provider platform needed by the template.
...
- **WHEN** an authenticated browser posts to the Host logout route
