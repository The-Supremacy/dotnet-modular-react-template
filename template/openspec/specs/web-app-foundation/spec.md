...
- **THEN** it submits logout to the Host logout route through a same-origin
  relative URL
...
- **THEN** current-user success, unauthenticated failure, login navigation, and
  logout POST submission behavior are verified without calling a real identity
  provider
...
- **AND** it includes a command that submits Host logout through a same-origin
  relative `/auth/` route
