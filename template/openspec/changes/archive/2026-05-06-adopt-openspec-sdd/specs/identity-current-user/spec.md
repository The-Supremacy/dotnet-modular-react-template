...
### Requirement: Transitional Store Boundary

A direct `IIdentityStore` abstraction MUST remain transitional only until repository/query-handler conventions are documented and implemented.

#### Scenario: Durable persistence conventions are introduced

- **WHEN** repository/query-handler conventions for Identity persistence are
  accepted
- **THEN** direct `IIdentityStore` usage is replaced or reshaped to match those
  conventions
