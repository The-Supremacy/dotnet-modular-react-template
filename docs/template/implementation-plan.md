...
- Make future agent work easier through short AGENTS indexes, durable docs, OpenSpec, and reusable scripts/skills.
...
- OpenSpec current specs describe accepted behavior.
- Planning notes remain clearly marked until converted into architecture docs, the template decision ledger, or OpenSpec artifacts.
...
- Likely first skills/scripts: initialize pinned OpenSpec tooling, add server module, run template rename/verification.
...
- OpenSpec Codex skills should be initialized from the `openspec init --tools codex` flow rather than hand-authored from scratch.
...
Use OpenSpec for spec-driven development that needs durable acceptance criteria,
current behavior specs, or cross-artifact planning. Do not use it for every
narrow architecture preference.

Decision: pivot from Spec Kit to OpenSpec after the first accepted behavior
slice. OpenSpec's active-change and current-spec model fits future maintenance
better than keeping capability knowledge in coarse archive memory files.

Initial OpenSpec stack:

- OpenSpec CLI, pinned to `@fission-ai/openspec@1.3.1`.
- Codex integration, initialized through `openspec init --tools codex`.
- Default `spec-driven` schema.

Do not auto-install OpenSpec from the devcontainer lifecycle. The devcontainer
should provide prerequisites such as Node.js and pnpm; a repo script should run
the actual OpenSpec initialization intentionally so generated files can be
reviewed.

Accepted current specs should stay behavior capability-oriented:

- Backend auth/session behavior.
- Identity current-user behavior.
- Host API behavior.

Governance, persistence-scope, implementation-structure, and verification-only
details belong in stable docs, task/design artifacts, or archived changes unless
they define observable system behavior.
...
- Behavior contracts go to OpenSpec changes first, then accepted behavior is archived into `openspec/specs/`.
- If docs, specs, and code drift, call out the drift before extending it.

Planning decisions such as folder shape, `.Infrastructure` projects, `.slnx` placement, testing categories, and rename automation do not need OpenSpec changes by default. Auth/session behavior, `/api/me`, admin bootstrap, and future admin provisioning workflows do.

The setup script is manual and reviewable under `scripts/setup-openspec.sh`.
It installs the pinned OpenSpec CLI, initializes Codex support, prints the
installed version, and refuses to reuse an existing `openspec/` setup unless
`--force` is provided.
...
9. Initial OpenSpec changes and current specs for auth/session, `/api/me`, and admin bootstrap.
...
- Use OpenSpec with Codex support for SDD.
